...
export default function App() {
  ...
  const fetchMints = async () => {
    const res = await fetch(
      'https://api.blockvision.org/v2/monad/collection/activities',
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ page: 1, pageSize: 20, method: 'mint' })
      }
    );
    const { data } = await res.json();
    setMints(data.activities || []);
  };
          ...
          <div key={i} className="border rounded-xl p-4 shadow">
            ...
            <div className="mt-2">
              ...
              <p className="text-sm text-gray-600">
                {dayjs(nft.timestamp * 1000).fromNow()}
              </p>
            </div>
          </div>
        ))}
